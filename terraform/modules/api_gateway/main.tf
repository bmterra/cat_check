...
resource "aws_apigatewayv2_api" "cats_check" {
...
## Single default Stage
resource "aws_apigatewayv2_stage" "default" {
  api_id      = aws_apigatewayv2_api.cats_check.id
  name        = "$default"
  auto_deploy = true
}

## We have 2 endpoints exposing Lambdas functions:
## s3_upload and cat_status
...
resource "aws_lambda_permission" "allow_api_gateway_invoke_s3_upload" {
  ...
  source_arn    = "${aws_apigatewayv2_api.cats_check.execution_arn}/*/*"
}
